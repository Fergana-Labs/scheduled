...
export default function BrandingToggle({ initialEnabled }: BrandingToggleProps) {
  ...
  async function toggle() {
    ...
    setSaving(true);
    try {
      await api('/web/api/v1/settings/branding', {
        method: 'PUT',
        body: JSON.stringify({ enabled: next }),
      });
      track('setting_changed', { setting: 'scheduled_branding_enabled', new_value: next });
    } catch {
      setEnabled(!next);
    } finally {
      setSaving(false);
