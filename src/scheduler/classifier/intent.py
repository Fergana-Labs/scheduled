...
def classify_sent_message_confirms_invite(message_body: str, event_summary: str) -> bool:
    """Classify whether a sent message still confirms a meeting invite.

    Used to guard pending invites — if the user rewrote the draft to decline
    or changed the topic entirely, we should not send the invite.

    Args:
        message_body: The body of the message the user actually sent.
        event_summary: The event summary from the pending invite.

    Returns:
        True if the message confirms/proposes the meeting, False if it
        declines, cancels, or is unrelated.
    """
    client = _get_anthropic_client()

    system_prompt = (
        "You are a classifier that determines whether a sent email still confirms "
        "or proposes a meeting.\n\n"
        "You will be given an email body and a meeting description. Decide whether "
        "the email is confirming, proposing, or accepting that meeting.\n\n"
        "Respond with a single JSON object only:\n"
        '{"confirms": true}  — if the email confirms/proposes/accepts the meeting\n'
        '{"confirms": false} — if the email declines, cancels, or is unrelated to the meeting\n'
        "No other output."
    )

    user_content = (
        f"Meeting: {event_summary}\n\n"
        f"Email body:\n{message_body}\n"
    )
    ...
    try:
        ...
        return bool(data.get("confirms", False))
    except Exception:
        # On failure, err on the side of NOT sending an unwanted invite
        return False
